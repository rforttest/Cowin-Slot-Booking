import json
import traceback
from PySimpleGUI.PySimpleGUI import Multiline, No, Print, pin 
...
# refresh_freq ha jo time wala hain
...
def collect_user_details(request_header):
    ...
    auto_book = input("Do you want to enable auto-booking? (y or no) Default no: ")
        ...
        for location in location_dtls:
            ...
            if resp.status_code == 401:
                ...
                if 'centers' in resp:
                    ...
                    # a = viable_options(resp, minimum_slots, min_age_booking, fee_type, dose)
        ...
        for location in location_dtls:
            ...
            if resp.status_code == 401:
                ...
                if 'centers' in resp:
                    ...
                    print("maxiumum calls exceed")
                    speak("maxium limit exceed")
                    time.sleep(20)
                    return False

            else:
                speak("Maxium LImit Excedded sir")
                print(resp.text)
                time.sleep(20)
                return False
...
def generate_captcha(request_header, captcha_automation):
    # try:
    #     threading.Thread(target=beep,args=(WARNING_BEEP_DURATION[0],WARNING_BEEP_DURATION[1])).start()
    # except Exception:
    #     speak("fast",rate=130)
    # # beep(WARNING_BEEP_DURATION[0], WARNING_BEEP_DURATION[1])
...
def book_appointment(request_header, details):
    ...
    try:
        ...
        while valid_captcha:
            ...
            if captcha == "post_error":
                speak("Cannot find captcha token invalid")
                return False
            
            
            elif captcha == 'None':
                speak("cannot automate captcha")
                speak("You Need To Enter capthcha manually")
                captcha = generate_captcha(request_header, 'n')
            
            
            elif captcha == None:
                speak("Error in captcha Automation")
                try:
                    captcha = generate_captcha(request_header, 'n')
                except Exception:
                    speak("Error occured in captcha file")
                    speak("Exiting System")
                    os.system("pause")

            else:
                # speak(f"The captcha is {captcha}")
    ...
    try:
        ...
        if len(tmp_options) > 0:
            ...
            try:
                ...
                for i in range(int(refresh_freq), 0, -1):
...
def generate_token_OTP(request_header):
    """
    ...
    while not valid_token:
        try:
            # ******** otp generation ****
            from login import get_authenticated_session
